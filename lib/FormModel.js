...
class FormModel {
    ...
    getDeploymentSubjects() {
        function getDeploymentSubs(deployment) {
            ...
            return lodash_1.default.union(approvers, deployment.enumerators, deployment.viewers, deployment.admins, deployment.superadmins || []);
        }
    ...
    amAdmin(user, groups) {
        ...
        return user == "admin" || lodash_1.default.any(this.form.roles, (r) => subjects.includes(r.id) && r.role === "admin");
    }
    /** Check if user is admin or deploy of entire form */
    amDeploy(user, groups) {
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
        return user == "admin" || lodash_1.default.any(this.form.roles, (r) => subjects.includes(r.id) && ["admin", "deploy"].includes(r.role));
    }
    /** Check if user is admin or superadmin of at least one deployment */
    amDeploymentAdmin(user, groups) {
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
        return user == "admin" || lodash_1.default.any(this.form.deployments, (dep) => lodash_1.default.intersection(lodash_1.default.union(dep.admins, dep.superadmins || []), subjects).length > 0);
    }
    /** Check if user is superadmin of at least one deployment */
    amDeploymentSuperadmin(user, groups) {
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
        return user == "admin" || lodash_1.default.any(this.form.deployments, (dep) => lodash_1.default.intersection(lodash_1.default.union(dep.superadmins || []), subjects).length > 0);
    }
    /** Check if user can edit deployment */
    canEditDeployment(deployment, user, groups) {
        if (this.amDeploy(user, groups)) {
            return true;
        }
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
        return lodash_1.default.intersection(deployment.superadmins || [], subjects).length > 0;
    }
}
