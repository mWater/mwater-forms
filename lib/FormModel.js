...
class FormModel {
    ...
    amDeploymentAdmin(user, groups) {
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
        return lodash_1.default.any(this.form.deployments, (dep) => lodash_1.default.intersection(lodash_1.default.union(dep.admins, dep.superadmins || []), subjects).length > 0);
    }
    /** Check if user is superadmin of at least one deployment */
    amDeploymentSuperadmin(user, groups) {
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
        return lodash_1.default.any(this.form.deployments, (dep) => lodash_1.default.intersection(lodash_1.default.union(dep.superadmins || []), subjects).length > 0);
    }
    /** Check if user can edit deployment */
    static canEditDeployment(deployment, user, groups) {
        let subjects = ["all", "user:" + user];
        subjects = subjects.concat(lodash_1.default.map(groups, (g) => "group:" + g));
