...
class ImageUploaderModalComponent extends react_1.default.Component {
    ...
    static show(apiUrl, client, T, success, forceCamera) {
        ModalPopupComponent_1.default.show((onClose) => {
    ...
    renderContents() {
        return R("div", null, R("form", { encType: "multipart/form-data", method: "post" }, R("label", { className: "btn btn-secondary btn-lg", style: { display: this.state.xhr ? "none" : undefined } }, R("span", { className: "fas fa-camera" }), " ", this.props.T("Select"), R("input", {
            type: "file",
            style: { display: "none" },
            accept: "image/*",
            capture: this.props.forceCamera ? "camera" : undefined,
            onChange: this.handleFileSelected
        })), R("div", { style: { display: !this.state.xhr ? "none" : undefined, marginBottom: 10 } }, R("p", null, R("em", null, this.props.T("Uploading Image..."))), R("div", { className: "progress progress-bar-striped active" }, R("div", { className: "progress-bar", style: { width: `${this.state.percentComplete}%` } })))), this.state.xhr
            ? R("button", { type: "button", className: "btn btn-secondary", onClick: this.handleCancel }, this.props.T("Cancel"))
            : undefined);
    }
    render() {
