...
utils = require('../utils');
initialDelay = 10000;
goodDelay = 5000; // Uses an algorithm to accurately find current position (coords + timestamp). Fires status events and found event. 
// Only call start once and be sure to call stop after

module.exports = CurrentPositionFinder =
/*#__PURE__*/
function () {
  function CurrentPositionFinder() {
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    (0, _classCallCheck2.default)(this, CurrentPositionFinder);
    ...
    this.afterGoodDelay = this.afterGoodDelay.bind(this); // Add events
  ...
  (0, _createClass2.default)(CurrentPositionFinder, [{
    key: "_reset",
    ...
    value: function start() {
      ...
      this.locationFinder.startWatch(); // Update status
    ...
    value: function locationFinderFound(pos) {
      var newStrength; // Calculate strength of new position

      newStrength = utils.calculateGPSStrength(pos); // If none, do nothing
      ...
      this.updateStatus(); // Start good delay if needed
    ...
    value: function updateStatus() {
      ...
      this.useable = this.initialDelayComplete && ((ref = this.strength) === "fair" || ref === "poor") || this.strength === "good"; // Trigger status
