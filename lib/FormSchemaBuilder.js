 ...
 */
var ColumnNotFoundException, ExprCompiler, ExprUtils, FormSchemaBuilder, Topo, _, appendStr, formUtils, mapTree, update;
...
module.exports = FormSchemaBuilder = (function() {
  ...
  FormSchemaBuilder.prototype.orderIndicatorCalculation = function(indicatorCalculations) {
    var col, expr, exprUtils, i, ic, id, j, len, len1, map, match, ref, refedColumns, refedIndicatorCalculationIds, toposort;
    toposort = new Topo();
    ...
    for (i = 0, len = indicatorCalculations.length; i < len; i++) {
      ...
      toposort.add(ic._id, {
        group: ic._id,
        after: refedIndicatorCalculationIds
      });
    }
    map = _.indexBy(indicatorCalculations, "_id");
    return _.map(toposort.nodes, function(id) {
      return map[id];
    });
  };
