...
module.exports = FormSchemaBuilder = (function() {
  ...
  FormSchemaBuilder.prototype.addForm = function(schema, form, cloneForms) {
    var conditionsExprCompiler, contents, deploymentValues;
    contents = [];
    ...
    this.addFormItem(form.design, contents, "responses:" + form._id, conditionsExprCompiler);
  ...
  FormSchemaBuilder.prototype.addFormItem = function(item, contents, tableId, conditionsExprCompiler, existingConditionExpr) {
    var addColumn, addCxColumn, answerType, cellCode, choice, code, codeExpr, column, conditionExpr, i, itemCode, itemColumn, itemItem, j, k, l, len, len1, len2, len3, len4, len5, m, n, name, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, results, section, sectionConditionExpr, sectionContents, sections, subitem, webmercatorLocation;
    ...
    if (item.contents) {
      if (item._type === "Form") {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          subitem = ref[i];
          results.push(this.addFormItem(subitem, contents, tableId, conditionsExprCompiler, existingConditionExpr));
        }
        return results;
        ...
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          subitem = ref2[j];
          this.addFormItem(subitem, sectionContents, tableId, conditionsExprCompiler, sectionConditionExpr);
        }
      ...
      switch (answerType) {
        ...
        case "site":
          ...
          column = {
            ...
            join: {
              type: "n-1",
              toTable: item.siteTypes ? "entities." + _.first(item.siteTypes).toLowerCase().replace(new RegExp(' ', 'g'), "_") : "entities.water_point",
              fromColumn: codeExpr,
              toColumn: "code"
            }
          };
      ...
      if (conditionsExprCompiler && ((item.conditions && item.conditions.length > 0) || existingConditionExpr)) {
