        ...
        for (let reverseJoin of reverseJoins) {
            ...
            if (schema.getTable(reverseJoin.table)) {
                ...
                if (sectionIndex < 0) {
                    ...
                    table = update_object_1.default(table, { contents: { $push: [section] } });
                    sectionIndex = lodash_1.default.findIndex(table.contents, (item) => item.id === "!related_forms");
                }
                // Add join
                section = update_object_1.default(table.contents[sectionIndex], { contents: { $push: [column] } });
                table = update_object_1.default(table, { contents: { $splice: [[sectionIndex, 1, section]] } });
                // Replace table
    ...
    addIndicatorCalculations(schema, form, indicators) {
        ...
        for (let indicatorCalculation of form.indicatorCalculations) {
            ...
            if (indicatorsSection.id !== "indicators") {
                ...
                schema = schema.addTable(update_object_1.default(schema.getTable(tableId), { contents: { $push: [indicatorsSection] } }));
            }
            ...
            contents[contents.length - 1] = update_object_1.default(indicatorsSection, { contents: { $set: indicatorSectionContents } });
            // Re-add table
            schema = schema.addTable(update_object_1.default(schema.getTable(tableId), { contents: { $set: contents } }));
        }
        return schema;
    }
                ...
                for (let rosterItem of item.contents) {
                    if (rosterItem.confidential) {
                        ...
                        if (!confidentialDataSection) {
                            ...
                            schema = schema.addTable(update_object_1.default(schema.getTable(tableId), { contents: { $push: [confidentialDataSection] } }));
                        }
                        ...
                        contents[index] = update_object_1.default(confidentialDataSection, { contents: { $set: confidentialDataSectionContents } });
                        schema = schema.addTable(update_object_1.default(schema.getTable(tableId), { contents: { $set: contents } }));
                    }
                }
            }
    ...
    addCalculations(schema, form) {
        ...
        for (let calculation of form.design.calculations) {
            ...
            if (calculationsSection.id !== "calculations") {
                ...
                schema = schema.addTable(update_object_1.default(schema.getTable(tableId), { contents: { $push: [calculationsSection] } }));
            }
            ...
            // Update in original
            const contents = schema.getTable(tableId).contents.slice();
            contents[contents.length - 1] = update_object_1.default(calculationsSection, { contents: { $set: calculationsSectionContents } });
            // Re-add table
            schema = schema.addTable(update_object_1.default(schema.getTable(tableId), { contents: { $set: contents } }));
        }
        return schema;
