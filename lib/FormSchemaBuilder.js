...
const ConditionsExprCompiler_1 = __importDefault(require("./ConditionsExprCompiler"));
const aquagenxCBTUtils_1 = require("./answers/aquagenxCBTUtils");
...
class FormSchemaBuilder {
    ...
    addForm(schema, form, cloneFormsDeprecated, isAdmin, indicators) {
        const contents = [];
        const metadata = [];
        // Get deployments
        const deploymentValues = lodash_1.default.map(form.deployments, (dep) => ({
            id: dep._id,
        ...
        const conditionsExprCompiler = new ConditionsExprCompiler_1.default(form.design);
    ...
    addReverseJoins(schema, form, reverseJoins) {
        for (let reverseJoin of reverseJoins) {
            const column = lodash_1.default.clone(reverseJoin.column);
            // Determine if is the only join to a table, in which case use the form name to be less confusing
            if (lodash_1.default.where(reverseJoins, { table: reverseJoin.table }).length === 1) {
            ...
            if (schema.getTable(reverseJoin.table)) {
                ...
                let sectionIndex = lodash_1.default.findIndex(table.contents, (item) => item.id === "!related_forms");
                if (sectionIndex < 0) {
                    ...
                    table = (0, update_object_1.default)(table, { contents: { $push: [section] } });
                    sectionIndex = lodash_1.default.findIndex(table.contents, (item) => item.id === "!related_forms");
                }
                // Add join
                section = (0, update_object_1.default)(table.contents[sectionIndex], { contents: { $push: [column] } });
                table = (0, update_object_1.default)(table, { contents: { $splice: [[sectionIndex, 1, section]] } });
                // Replace table
    ...
    addIndicatorCalculations(schema, form, indicators) {
        ...
        for (let indicatorCalculation of form.indicatorCalculations) {
            ...
            let indicatorsSection = lodash_1.default.last(schema.getTable(tableId).contents);
            if (indicatorsSection.id !== "indicators") {
                ...
                schema = schema.addTable((0, update_object_1.default)(schema.getTable(tableId), { contents: { $push: [indicatorsSection] } }));
            }
            ...
            contents[contents.length - 1] = (0, update_object_1.default)(indicatorsSection, { contents: { $set: indicatorSectionContents } });
            // Re-add table
            schema = schema.addTable((0, update_object_1.default)(schema.getTable(tableId), { contents: { $set: contents } }));
        }
        return schema;
    }
    ...
    createIndicatorCalculationSection(indicatorCalculation, schema, indicators, form) {
        // Find indicator
        const indicator = lodash_1.default.findWhere(indicators, { _id: indicatorCalculation.indicator });
        ...
        // Map properties
        const contents = [];
        for (let properties of lodash_1.default.values(indicator.design.properties)) {
            for (let property of properties) {
                ...
                const column = lodash_1.default.extend({}, property, {
            ...
            if (item._type == "RosterGroup" || item._type == "RosterMatrix") {
                ...
                for (let rosterItem of item.contents) {
                    if (rosterItem.confidential) {
                        let confidentialDataSection = lodash_1.default.find(contents, { id: "confidentialData" });
                        if (!confidentialDataSection) {
                            ...
                            schema = schema.addTable((0, update_object_1.default)(schema.getTable(tableId), { contents: { $push: [confidentialDataSection] } }));
                        }
                        ...
                        const index = lodash_1.default.findIndex(contents, { id: "confidentialData" });
                        contents[index] = (0, update_object_1.default)(confidentialDataSection, { contents: { $set: confidentialDataSectionContents } });
                        schema = schema.addTable((0, update_object_1.default)(schema.getTable(tableId), { contents: { $set: contents } }));
                    }
                }
            }
    ...
    addConfidentialData(schema, form, conditionsExprCompiler) {
        ...
        const addData = (question) => {
            if (question.confidential) {
                let confidentialDataSection = lodash_1.default.find(schema.getTable(tableId).contents, { id: "confidentialData" });
                if (!confidentialDataSection) {
                    confidentialDataSection = {
                        id: "confidentialData",
                        type: "section",
                ...
                const confidentialDataSectionContents = confidentialDataSection.contents.slice();
                this.addFormItem(question, confidentialDataSectionContents, `responses:${form._id}`, conditionsExprCompiler, null, [], true);
                // Update in original
                const contents = schema.getTable(tableId).contents.slice();
                const index = lodash_1.default.findIndex(contents, { id: "confidentialData" });
                contents[index] = (0, update_object_1.default)(confidentialDataSection, { contents: { $set: confidentialDataSectionContents } });
        ...
        if (item.contents) {
            if (item._type === "Form") {
                ...
                if (conditionsExprCompiler) {
                    sectionConditionExpr = mwater_expressions_1.ExprUtils.andExprs(tableId, existingConditionExpr, conditionsExprCompiler.compileConditions(item.conditions, tableId));
                }
                else {
            ...
            switch (answerType) {
                ...
                case "choice":
                    ...
                    column = {
                        ...
                        name: item.text,
                        code,
                        enumValues: lodash_1.default.map(item.choices, (c) => ({
                            id: c.id,
                            name: c.label,
                            code: c.code
                        })),
                ...
                case "units":
                    ...
                    column = {
                        ...
                        enumValues: lodash_1.default.map(item.units, (c) => ({
                            id: c.id,
                            name: c.label
                        }))
                    };
                ...
                case "aquagenx_cbt":
                    ...
                    section.contents.push({
                        ...
                        type: "enum",
                        enumValues: aquagenxCBTUtils_1.healthRiskEnum,
                        ...
                        for (let itemColumn of item.columns) {
                            ...
                            if (itemColumn._type === "DropdownColumnQuestion") {
                                section.contents.push({
                                    ...
                                    name: appendStr(appendStr(appendStr(appendStr(item.text, ": "), itemItem.label), " - "), itemColumn.text),
                                    code: cellCode,
                                    enumValues: lodash_1.default.map(itemColumn.choices, (c) => ({
                                        id: c.id,
                                        code: c.code,
                                        name: c.label
                                    })),
                            ...
                            if (itemColumn._type === "UnitsColumnQuestion") {
                                ...
                                section.contents.push({
                                    ...
                                    enumValues: lodash_1.default.map(itemColumn.units, (c) => ({
                                        id: c.id,
                                        code: c.code,
                                        name: c.label
                                    })),
                ...
                case "ranked":
                    ...
                    for (const choice of item.choices) {
                        ...
                        section.contents.push({
                            ...
                            code: itemCode,
                            enumValues: item.choices.map((choice, index) => ({ id: String(index + 1), name: { _base: "en", en: String(index + 1) } })),
                            jsonql: {
                                type: "op",
                                op: "#>>",
                                exprs: [
            ...
            if (item.randomAskProbability == null &&
                ...
                // Guard against null
                let conditionExpr = mwater_expressions_1.ExprUtils.andExprs(tableId, existingConditionExpr, conditionsExprCompiler.compileConditions(item.conditions, tableId));
                if (conditionExpr) {
                    conditionExpr = {
                        type: "op",
                        op: "and",
    ...
    addCalculations(schema, form) {
        ...
        for (let calculation of form.design.calculations) {
            ...
            let calculationsSection = lodash_1.default.last(schema.getTable(tableId).contents);
            if (calculationsSection.id !== "calculations") {
                ...
                schema = schema.addTable((0, update_object_1.default)(schema.getTable(tableId), { contents: { $push: [calculationsSection] } }));
            }
            ...
            // Update in original
            const contents = schema.getTable(tableId).contents.slice();
            contents[contents.length - 1] = (0, update_object_1.default)(calculationsSection, { contents: { $set: calculationsSectionContents } });
            // Re-add table
            schema = schema.addTable((0, update_object_1.default)(schema.getTable(tableId), { contents: { $set: contents } }));
        }
        return schema;
...
exports.default = FormSchemaBuilder;
...
function appendStr(str, suffix) {
    ...
    for (let key in str) {
        ...
        if (key === "_base") {
            ...
            if (lodash_1.default.isString(suffix)) {
...
function mapTree(tree, func) {
    ...
    if (lodash_1.default.isArray(tree)) {
        return lodash_1.default.map(tree, (item) => mapTree(item, func));
    }
    // Map item
    const output = func(tree);
    // Map contents
    if (tree.contents) {
        output.contents = lodash_1.default.compact(lodash_1.default.map(tree.contents, (item) => func(item)));
    }
    return output;
}
...
function formizeIndicatorPropertyExpr(expr, form, indicatorCalculation, indicator) {
    ...
    if (!lodash_1.default.isObject(expr)) {
        return expr;
    }
    ...
    if (expr.table === `indicator_values:${indicator._id}`) {
        expr = Object.assign(Object.assign({}, expr), { table: `responses:${form._id}` });
    }
    // Otherwise replace recursively
    return lodash_1.default.mapValues(expr, function (value, key) {
        if (lodash_1.default.isArray(value)) {
            return lodash_1.default.map(value, (v) => formizeIndicatorPropertyExpr(v, form, indicatorCalculation, indicator));
        }
        else {
...
function createWebmercatorGeometry(dataColumn, latPath, lngPath, 
