...
class FormSchemaBuilder {
            ...
            switch (answerType) {
                ...
                case "asset":
                    column = {
                        ...
                        join: {
                            type: "n-1",
                            toTable: `assets:${item.assetSystemId}`,
                            fromColumn: {
                                type: "op",
                                op: "#>>",
                                exprs: [{ type: "field", tableAlias: "{alias}", column: dataColumn }, `{${item._id},value}`]
                    ...
                    // Add reverse join 
                    reverseJoin = {
                        table: `assets:${item.assetSystemId}`,
                        column: {
                            id: `${tableId}:data:${item._id}:value`,
                            // Form name is not available here. Prefix later.
                            name: item.text,
    ...
    addCalculations(schema, form) {
        ...
        for (let calculation of form.design.calculations) {
            ...
            const exprType = new mwater_expressions_1.ExprUtils(schema).getExprType(calculation.expr) || "number";
