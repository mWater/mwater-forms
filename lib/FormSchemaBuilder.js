...
module.exports = FormSchemaBuilder = (function() {
  ...
  FormSchemaBuilder.prototype.addForm = function(schema, form, cloneForms) {
    ...
    this.addFormItem(form, form.design, contents, "responses:" + form._id);
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      if ((ref1 = item._type) === "RosterGroup" || ref1 === "RosterMatrix") {
        ...
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          rosterItem = ref2[j];
          this.addFormItem(form, rosterItem, contents, "responses:" + form._id + ":roster:" + (item.rosterId || item._id));
        }
        schema = schema.addTable({
  ...
  FormSchemaBuilder.prototype.addFormItem = function(form, item, contents, tableId) {
    ...
    if (item.contents) {
      if (item._type === "Form") {
        ...
        for (i = 0, len = ref.length; i < len; i++) {
          subitem = ref[i];
          results.push(this.addFormItem(form, subitem, contents, tableId));
        }
        return results;
        ...
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          subitem = ref2[j];
          this.addFormItem(form, subitem, sectionContents, tableId);
        }
