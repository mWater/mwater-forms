var ColumnNotFoundException, ConditionsExprCompiler, ExprCompiler, ExprUtils, FormSchemaBuilder, TopoSort, _, appendStr, formUtils, formizeIndicatorPropertyExpr, healthRiskEnum, mapTree, update;
...
module.exports = FormSchemaBuilder = (function() {
  ...
  FormSchemaBuilder.prototype.addForm = function(schema, form, cloneForms, isAdmin, indicators) {
    ...
    schema = this.addRosterTables(schema, form, conditionsExprCompiler, reverseJoins);
    ...
    if (form.isMaster) {
      schema = this.addMasterForm(schema, form, cloneForms, indicators);
    }
    return schema;
  };
  ...
  FormSchemaBuilder.prototype.addRosterTables = function(schema, form, conditionsExprCompiler, reverseJoins) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      if ((ref1 = item._type) === "RosterGroup" || ref1 === "RosterMatrix") {
        ...
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          rosterItem = ref2[j];
          this.addFormItem(rosterItem, contents, "responses:" + form._id + ":roster:" + (item.rosterId || item._id), conditionsExprCompiler, null, reverseJoins);
        }
        schema = schema.addTable({
  ...
  FormSchemaBuilder.prototype.addMasterForm = function(schema, form, cloneForms, indicators) {
    ...
    return schema = this.addIndicatorCalculations(schema, form, indicators, true);
  };

  FormSchemaBuilder.prototype.addIndicatorCalculations = function(schema, form, indicators, isMaster) {
    ...
    for (i = 0, len = ref.length; i < len; i++) {
      ...
      indicatorCalculationSection = this.createIndicatorCalculationSection(indicatorCalculation, schema, indicators, isMaster, form);
  ...
  FormSchemaBuilder.prototype.createIndicatorCalculationSection = function(indicatorCalculation, schema, indicators, isMaster, form) {
    var column, condition, contents, exprCompiler, expression, fromColumn, i, indicator, j, len, len1, properties, property, ref, section, toColumn;
...
formizeIndicatorPropertyExpr = function(expr, form, indicatorCalculation, indicator) {
  if (!expr) {
    return expr;
  }
  if (!_.isObject(expr)) {
  ...
  return _.mapValues(expr, function(value, key) {
    if (_.isArray(value)) {
      return _.map(value, function(v) {
        return formizeIndicatorPropertyExpr(v, form, indicatorCalculation, indicator);
      });
    } else {
