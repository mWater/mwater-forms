...
module.exports = FormSchemaBuilder = (function() {
  function FormSchemaBuilder(options) {
    if (options == null) {
      options = {};
    }
    this.roles = ['all'];
    if (options.user) {
      this.roles.push("user:" + options.user);
    }
    this.roles = this.roles.concat(_.map(options.groups, function(g) {
      return "group:" + g;
    }));
  }

  FormSchemaBuilder.prototype.addForm = function(schema, form, cloneForms) {
    var conditionsExprCompiler, contents, deploymentValues, matchingRoles, metadata, reverseJoins;
    contents = [];
    metadata = [];
    ...
    matchingRoles = _.compact(_.map(this.roles, function(role) {
      return _.find(form.roles, {
        id: role
      });
    }));
    if (_.some(matchingRoles, {
      role: 'admin'
    })) {
