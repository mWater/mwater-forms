...
var ColumnNotFoundException, ConditionsExprCompiler, ExprCompiler, ExprUtils, FormSchemaBuilder, TopoSort, _, appendStr, createWebmercatorGeometry, formUtils, _formizeIndicatorPropertyExpr, healthRiskEnum, _mapTree, update;
...
module.exports = FormSchemaBuilder = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(FormSchemaBuilder, [{
    key: "addForm",
    value: // indicators is at least all indicators referenced in indicator calculations. Can be empty and indicator calculations will be omitted
    function addForm(schema, form, cloneFormsDeprecated) {
    ...
    value: function addFormItem(item, contents, tableId, conditionsExprCompiler, existingConditionExpr) {
      ...
      if (item.contents) {
        ...
        switch (answerType) {
          ...
          case "location":
            column = {
              ...
              name: item.text,
              code: code,
              jsonql: createWebmercatorGeometry(dataColumn, "{".concat(item._id, ",value,latitude}"), "{".concat(item._id, ",value,longitude}"), "{alias}")
            };
            addColumn(column);
            column = {
            ...
            if (item.calculateAdminRegion) {
              ...
              webmercatorLocation = createWebmercatorGeometry(dataColumn, "{".concat(item._id, ",value,latitude}"), "{".concat(item._id, ",value,longitude}"), "{from}");
              column = {
          ...
          case "site":
            ...
            siteType = (item.siteTypes ? item.siteTypes[0] : void 0) || "water_point"; // Site column question have siteType
          ...
          case "matrix":
            ...
            for (m = 0, len4 = ref6.length; m < len4; m++) {
              itemItem = ref6[m]; // Create section
              ...
              ref7 = item.columns; // For each column
        ...
        if (item.commentsField) {
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":comments"),
            type: "text",
        ...
        if (item.recordTimestamp) {
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":timestamp"),
            type: "datetime",
        ...
        if (item.recordLocation) {
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":location"),
            type: "geometry",
            name: appendStr(item.text, " (Location Answered)"),
            code: code ? code + " (Location Answered)" : void 0,
            jsonql: createWebmercatorGeometry("data", "{".concat(item._id, ",location,latitude}"), "{".concat(item._id, ",location,longitude}"), "{alias}")
          };
          addColumn(column);
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":location:accuracy"),
            type: "number",
          ...
          addColumn(column);
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":location:altitude"),
            type: "number",
        ...
        if (item.alternates && item.alternates.na) {
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":na"),
            type: "boolean",
        ...
        if (item.alternates && item.alternates.dontknow) {
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":dontknow"),
            type: "boolean",
        ...
        if (item.randomAskProbability != null) {
          column = {
            id: "".concat(dataColumn, ":").concat(item._id, ":randomAsked"),
            type: "boolean",
        ...
        if (item.randomAskProbability == null && conditionsExprCompiler && (item.conditions && item.conditions.length > 0 || existingConditionExpr)) {
          ...
          if (conditionExpr) {
            ...
            column = {
              id: "".concat(dataColumn, ":").concat(item._id, ":visible"),
              type: "boolean",
...
createWebmercatorGeometry = function createWebmercatorGeometry(dataColumn, latPath, lngPath, tableAlias) {
  return {
