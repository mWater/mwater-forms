var Backbone, EntityDisplayComponent, EntityLoadingComponent, FormCompiler, H, ImageDisplayComponent, React, ResponseDisplayComponent, formUtils, moment,
...
module.exports = ResponseDisplayComponent = (function(superClass) {
  ...
  ResponseDisplayComponent.prototype.renderEvent = function(ev) {
    ...
    return H.div(null, eventType, " ", this.props.T("by"), " ", ev.by, " ", this.props.T("on"), " ", moment(ev.on).format('lll'), ev.message ? ": " + ev.message : void 0, ev.override ? H.span({
      className: "label label-warning"
    }, this.props.T("Admin Override")) : void 0);
  };

  ResponseDisplayComponent.prototype.renderHistory = function() {
    var contents, ev, i, lastEvent, len, ref;
    contents = [];
    if (this.state.showCompleteHistory) {
      ref = this.props.response.events || [];
      for (i = 0, len = ref.length; i < len; i++) {
        ev = ref[i];
        contents.push(this.renderEvent(ev));
      }
    }
    lastEvent = _.last(this.props.response.events || []);
    if (lastEvent) {
  ...
  ResponseDisplayComponent.prototype.renderHeader = function() {
    return H.div(null, H.div({
      ...
      key: "date"
    }, this.props.T('Date'), ": ", H.b(null, moment(this.props.response.modified.on).format('lll'))), this.renderHistory());
  };
