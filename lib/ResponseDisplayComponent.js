...
module.exports = ResponseDisplayComponent = (function(superClass) {
  ...
  ResponseDisplayComponent.prototype.componentWillMount = function() {
    var byArray, events, url;
    events = this.props.response.events || [];
    if (events.length > 0 && (this.props.apiUrl != null)) {
      byArray = _.map(events, function(event) {
        return "\"" + event.by + "\"";
      });
      url = this.props.apiUrl + 'users_public_data?filter={"_id":{"$in":[' + byArray.join(',') + ']}}';
      this.setState({
  ...
  ResponseDisplayComponent.prototype.componentWillReceiveProps = function(nextProps) {
    var events;
    if (this.props.form.design !== nextProps.form.design) {
      this.setState({
        formExprEvaluator: new FormExprEvaluator(nextProps.form.design)
      });
    }
  ...
  ResponseDisplayComponent.prototype.renderEvent = function(ev) {
    ...
    return H.div(null, eventType, " ", this.state.T("by"), " ", this.state.eventsUsernames[ev.by].username, " ", this.state.T("on"), " ", moment(ev.on).format('lll'), ev.message ? [": ", H.i(null, ev.message)] : void 0, ev.override ? H.span({
  ...
  ResponseDisplayComponent.prototype.renderHeader = function() {
    return H.div({
      ...
      key: "user"
    }, this.state.T('User'), ": ", H.b(null, this.props.response.username)), H.div({
      key: "code"
  ...
  ResponseDisplayComponent.prototype.render = function() {
    return H.div(null, this.renderHeader(), React.createElement(ResponseAnswersComponent, {
      ...
      displayMap: this.props.formCtx.displayMap
    }));
  };
