...
module.exports = ResponseDisplayComponent = (function(superClass) {
  ...
  ResponseDisplayComponent.prototype.renderColumnQuestion = function(q) {
    var answer;
    answer = this.props.response.data[q._id];
    return H.tr({
      key: q._id
  ...
  ResponseDisplayComponent.prototype.renderItem = function(item, visibilityStructure) {
    if (!visibilityStructure[item._id]) {
      return;
    }
    if (item._type === "Section" || item._type === "Group" || item._type === "RosterGroup") {
      return [
        H.tr({
          key: item._id
        }, H.td({
          ...
          return function(item) {
            return _this.renderItem(item, visibilityStructure);
          };
        })(this))
      ];
    }
    ...
    if (formUtils.isQuestion(item)) {
      return this.renderColumnQuestion(item);
    }
  };
