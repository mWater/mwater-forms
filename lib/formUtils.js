 ...
 */
function findAssetQuestion(formDesign, assetSystemId, assetType) {
    var _a;
...
exports.findAssetQuestion = findAssetQuestion;
 ...
 */
function getExtraTablesReferenced(formDesign) {
    const items = allItems(formDesign);
    const tableIds = [];
    const cascadingRefQuestions = items.filter(item => item._type === "CascadingRefQuestion");
    ...
    const assetQuestions = items.filter(item => item._type === "AssetQuestion");
...
exports.getExtraTablesReferenced = getExtraTablesReferenced;
