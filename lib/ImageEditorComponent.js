var H, ImageEditorComponent, ImagePopupComponent, ImageThumbnailComponent, React,
  bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
  extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
  hasProp = {}.hasOwnProperty;

React = require('react');

H = React.DOM;

ImageThumbnailComponent = require('./ImageThumbnailComponent');

ImagePopupComponent = require('./ImagePopupComponent');

module.exports = ImageEditorComponent = (function(superClass) {
  extend(ImageEditorComponent, superClass);

  ImageEditorComponent.propTypes = {
    imageManager: React.PropTypes.object.isRequired,
    imageAcquirer: React.PropTypes.object,
    image: React.PropTypes.object,
    onImageChange: React.PropTypes.func
  };

  function ImageEditorComponent() {
    this.handleAdd = bind(this.handleAdd, this);
    this.handleClickImage = bind(this.handleClickImage, this);
    ImageEditorComponent.__super__.constructor.apply(this, arguments);
    this.state = {
      modal: null
    };
  }

  ImageEditorComponent.prototype.handleClickImage = function() {
    var modal;
    modal = React.createElement(ImagePopupComponent, {
      imageManager: this.props.imageManager,
      id: this.props.image.id,
      onRemove: (function(_this) {
        return function() {
          _this.setState({
            modal: null
          });
          return _this.props.onImageChange(null);
        };
      })(this),
      onClose: (function(_this) {
        return function() {
          return _this.setState({
            modal: null
          });
        };
      })(this)
    });
    return this.setState({
      modal: modal
    });
  };

  ImageEditorComponent.prototype.handleAdd = function() {
    return this.props.imageAcquirer.acquire((function(_this) {
      return function(id) {
        return _this.props.onImageChange({
          id: id
        });
      };
    })(this), (function(_this) {
      return function(err) {
        throw err;
      };
    })(this));
  };

  ImageEditorComponent.prototype.render = function() {
    return H.div(null, this.state.modal, this.props.image ? React.createElement(ImageThumbnailComponent, {
      imageId: this.props.image.id,
      onClick: this.handleClickImage,
      imageManager: this.props.imageManager
    }) : this.props.onImageChange && this.props.imageAcquirer ? H.img({
      src: "data:image/png;base64,",
      className: "img-rounded",
      onClick: this.handleAdd,
      style: {
        maxHeight: 100
      }
    }) : H.div({
      className: "text-muted"
    }, T("No images present")));
  };

  return ImageEditorComponent;

})(React.Component);
