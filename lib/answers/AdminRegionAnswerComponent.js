    ...
    AdminRegionAnswerComponent = class AdminRegionAnswerComponent extends react_1.default.Component {
        ...
        renderEntityButtons() {
            return R("div", null, !this.state.waiting
                ...
                }, R("span", { className: "glyphicon glyphicon-screenshot" }), " ", this.context.T("Set Using GPS"))
                : R("button", {
                    type: "button",
                    className: "btn btn-link btn-sm",
                    onClick: this.handleCancelUseGPS,
                    disabled: this.context.locationFinder == null
                }, R("span", { className: "glyphicon glyphicon-remove" }), " ", this.context.T("Cancel GPS")), R("button", {
                type: "button",
                className: "btn btn-link btn-sm",
                onClick: this.handleUseMap,
                disabled: this.context.displayMap == null
            }, R("span", { className: "glyphicon glyphicon-map-marker" }), " ", this.context.T("Set Using Map")));
        }
        render() {
