...
var CascadingRefAnswerComponent = /** @class */ (function (_super) {
    ...
    function CascadingRefAnswerComponent(props) {
        ...
        _this.handleChange = function (index, value) {
            var dropdowns = _this.props.question.dropdowns;
            // If first one reset, then reset entire control
            if (index === 0 && !value) {
                _this.handleReset();
                return;
            }
            var dropdownValues = _this.state.dropdownValues.slice();
            ...
            pos = index + 1;
            for (; pos < dropdowns.length; pos++) {
                var values = _this.findValues(pos, dropdownValues);
                if (values.length == 1) {
            ...
            // Set rest to null
            for (; pos < _this.props.question.dropdowns.length; pos++) {
                dropdownValues[pos] = null;
            }
            ...
            if (dropdownValues[dropdowns.length - 1]) {
                ...
                for (var _i = 0, _a = _this.state.rows; _i < _a.length; _i++) {
                    var row = _a[_i];
                    var exclude = false;
                    for (var pos_1 = 0; pos_1 < dropdowns.length; pos_1++) {
                        if (row[dropdowns[pos_1].columnId] !== dropdownValues[pos_1]) {
                            exclude = true;
                        }
                    }
                    if (!exclude) {
                        // No longer editing
                        _this.setState({ dropdownValues: dropdownValues, editing: false });
                        _this.props.onValueChange(row._id);
                        return;
                    }
                }
            }
            _this.setState({ dropdownValues: dropdownValues, editing: true });
        };
        /** Reset control */
        _this.handleReset = function () {
            _this.setState({ dropdownValues: _this.props.question.dropdowns.map(function (c) { return null; }), editing: false });
    ...
    CascadingRefAnswerComponent.prototype.componentDidMount = function () {
        ...
        this.props.getCustomTableRows(this.props.question.tableId).then(function (rows) {
            ...
            // Get dropdown values
            var dropdownValues = _this.props.question.dropdowns.map(function (sel) { return row ? row[sel.columnId] : null; });
            _this.setState({ dropdownValues: dropdownValues, rows: rows });
        }).catch(function (err) {
            _this.setState({ rows: [] });
            throw err;
        });
    ...
    CascadingRefAnswerComponent.prototype.findValues = function (index, dropdownValues) {
        var values = [];
    ...
    CascadingRefAnswerComponent.prototype.renderDropdown = function (index) {
        var dropdown = this.props.question.dropdowns[index];
        // Determine available options
        var options = [];
        // Find all possible values, filtering by all previous selections
        var values = this.findValues(index, this.state.dropdownValues);
        var table = this.props.schema.getTable(this.props.question.tableId);
        if (!table) {
            // Means that doesn't have access to table
            return react_1.default.createElement("div", { className: "alert alert-danger" }, this.props.T("Unable to access data. Please check permissions and internet connection."));
        }
        var column = this.props.schema.getColumn(this.props.question.tableId, dropdown.columnId);
        if (!column) {
        ...
        return react_1.default.createElement("div", { style: { paddingBottom: 15 }, key: index },
            ...
            dropdown.hint ?
                react_1.default.createElement("div", { className: "text-muted" }, formUtils_1.localizeString(dropdown.hint, this.props.locale))
                : null);
    };
    CascadingRefAnswerComponent.prototype.render = function () {
        ...
        var dropdowns = [];
        for (var i = 0; i < this.props.question.dropdowns.length; i++) {
        ...
        return react_1.default.createElement("div", null,
            this.state.rows.length == 0 ?
                react_1.default.createElement("div", { className: "alert alert-warning" }, this.props.T("No data found for question. Please ensure that you are connected to internet for the first use of this form."))
                : null,
