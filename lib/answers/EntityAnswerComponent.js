...
class EntityAnswerComponent extends AsyncLoadComponent_1.default {
    ...
    renderEntityButtons() {
        return R("div", null, R("button", { type: "button", className: "btn btn-link btn-sm", onClick: this.handleSelectEntity }, R("span", { className: "fas fa-check" }), " ", this.context.T("Change Selection")), R("button", { type: "button", className: "btn btn-link btn-sm", onClick: this.handleClearEntity }, R("span", { className: "fas fa-times" }), " ", this.context.T("Clear Selection")), this.context.editEntity != null && this.context.canEditEntity(this.props.entityType, this.state.entity)
            ? R("button", { type: "button", className: "btn btn-link btn-sm", onClick: this.handleEditEntity }, R("span", { className: "fas fa-pencil-alt" }), " ", this.context.T("Edit Selection"))
            : undefined);
    }
    render() {
        ...
        if (!this.props.value) {
            ...
            return R("button", { type: "button", className: "btn btn-secondary btn-sm", onClick: this.handleSelectEntity }, R("span", { className: "fas fa-check" }), " ", this.context.T("Select"));
        }
