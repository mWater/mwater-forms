...
module.exports = FormCompiler = (function() {
      ...
      return function() {
        ...
        if (form) {
          isQuestionVisible = function(questionId) {
            ...
            return _this.compileConditions(question.conditions)();
          };
        } else {
  ...
  FormCompiler.prototype.compileForm = function(form) {
    ...
    options.setEntity = (function(_this) {
      return function(entityType, entity, questionId) {
        var entry, question;
        if (form.entitySettings) {
          formViewEntity = entity;
          _this.compileLoadLinkedAnswers(form.entitySettings.propertyLinks)(entity);
          return;
        }
        if (questionId) {
          question = formUtils.findItem(form, questionId);
        } else {
          question = _.find(formUtils.priorQuestions(form), function(q) {
            return q._type === "EntityQuestion" && q.entityType === entityType;
          });
        }
        if (!question) {
        ...
        _this.compileLoadLinkedAnswers(question.propertyLinks)(entity);
        entry = _this.model.get(question._id) || {};
        entry = _.clone(entry);
        entry.value = entity._id;
        return _this.model.set(question._id, entry);
      };
    })(this);
    options.getEntityCreates = (function(_this) {
      return function() {
        var creates, entity, i, len1, question, ref;
        creates = [];
        if (form.entitySettings && (formViewEntity == null)) {
          creates.push({
            type: _.last(form.entitySettings.entityType.split(":")),
            entity: _this.compileSaveLinkedAnswers(form.entitySettings.propertyLinks)()
          });
        }
        ref = formUtils.priorQuestions(form);
        for (i = 0, len1 = ref.length; i < len1; i++) {
          ...
          if (question._type === "EntityQuestion" && question.propertyLinks && question.createEntity) {
            if (!_this.model.get(question._id) || !_this.model.get(question._id).value) {
              entity = _this.compileSaveLinkedAnswers(question.propertyLinks)();
              creates.push({
    ...
    options.getEntityUpdates = (function(_this) {
      return function() {
        var i, len1, propertyUpdates, question, ref, updates;
        updates = [];
        if (form.entitySettings && (formViewEntity != null)) {
          updates.push({
        ...
        for (i = 0, len1 = ref.length; i < len1; i++) {
          ...
          if (question._type === "EntityQuestion" && question.propertyLinks) {
            if (_this.model.get(question._id) && _this.model.get(question._id).value) {
              propertyUpdates = _this.compileSaveLinkedAnswers(question.propertyLinks)();
    ...
    options.markEntityCreated = function(questionId, entity) {
      var entry;
      entry = this.model.get(questionId) || {};
      entry = _.clone(entry);
      entry.value = entity._id;
      return this.model.set(questionId, entry);
    };
