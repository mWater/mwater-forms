...
const formUtils_1 = __importDefault(require("./formUtils"));
...
class ResponseDataExprValueUpdater {
    constructor(formDesign, schema, dataSource) {
        ...
        this.formItems = {};
        for (let item of formUtils_1.default.allItems(this.formDesign)) {
            if (item._id) {
    ...
    validateData(data, responseRow, callback) {
        ...
        return new Promise((resolve, reject) => {
            visibilityCalculator.createVisibilityStructure(data, responseRow, (error, visibilityStructure) => {
                ...
                new ResponseDataValidator_1.default().validate(this.formDesign, visibilityStructure, data, this.schema, responseRow).then(resolve).catch(reject);
            });
        });
    }
    ...
    updateDataMultiple(data, exprValues) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const scalarEvs = exprValues.filter(ev => ev.expr.type == "scalar").map(ev => ({ expr: ev.expr, value: ev.value }));
    ...
    updateData(data, expr, value, callback) {
        ...
        if ((expr.type === "field") && expr.column.match(/^data:[^:]+:value:.+$/)) {
            ...
            const answerType = formUtils_1.default.getAnswerType(question);
