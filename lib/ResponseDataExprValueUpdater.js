var ExprCompiler, ResponseCleaner, ResponseDataExprValueUpdater, ResponseDataValidator, VisibilityCalculator, formUtils;
...
module.exports = ResponseDataExprValueUpdater = (function() {
  ...
  ResponseDataExprValueUpdater.prototype.canUpdate = function(expr) {
    ...
    if (expr.type === "field") {
      ...
      if (expr.type === "op" && ((ref = expr.op) === 'latitude' || ref === 'longitude') && expr.expr.type === "field" && expr.expr.column.match(/^data:.+:value$/)) {
        return true;
      }
    ...
    if (expr.type === "op" && ((ref1 = expr.op) === 'latitude' || ref1 === 'longitude') && expr.exprs[0].type === "field" && expr.exprs[0].column.match(/^data:[^:]+:value$/)) {
      return true;
    }
    if (expr.type === "scalar" && expr.joins.length === 1 && !expr.aggr && expr.joins[0].match(/^data:.+$/)) {
      return true;
    }
    if (expr.type === "op" && expr.op === "contains" && expr.exprs[0].type === "field" && expr.exprs[0].column.match(/^data:[^:]+:value$/) && ((ref2 = expr.exprs[1].value) != null ? ref2.length : void 0) === 1) {
      return true;
    }
    return false;
  };

  ResponseDataExprValueUpdater.prototype.cleanData = function(data) {
    ...
    data = responseCleaner.cleanData(data, visibilityStructure, this.formDesign);
    return data;
  };

  ResponseDataExprValueUpdater.prototype.validateData = function(data) {
    var result;
    result = new ResponseDataValidator().validate(this.formDesign, data);
    if (result) {
