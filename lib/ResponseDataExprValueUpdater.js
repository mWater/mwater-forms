...
module.exports = ResponseDataExprValueUpdater = (function() {
  ...
  ResponseDataExprValueUpdater.prototype.cleanData = function(data, createResponseRow, callback) {
    ...
    return responseCleaner.cleanData(this.formDesign, visibilityCalculator, null, randomAskedCalculator, data, createResponseRow, null, (function(_this) {
  ...
  ResponseDataExprValueUpdater.prototype.updateData = function(data, expr, value, callback) {
    ...
    if (expr.type === "field" && expr.column.match(/^data:[^:]+:value:cbt:mpn$/)) {
      this.updateCBTMPN(data, expr, value, callback);
      return;
    }
    if (expr.type === "field" && expr.column.match(/^data:[^:]+:value:cbt:confidence$/)) {
      this.updateCBTConfidence(data, expr, value, callback);
      return;
    }
    if (expr.type === "field" && expr.column.match(/^data:[^:]+:value:cbt:healthRisk$/)) {
      this.updateCBTHealthRisk(data, expr, value, callback);
      return;
    }
  ...
  ResponseDataExprValueUpdater.prototype.updateCBTMPN = function(data, expr, value, callback) {
    var answer, cbt, question, ref;
    question = this.formItems[expr.column.match(/^data:([^:]+):value:cbt:mpn$/)[1]];
    if (!question) {
      return callback(new Error("Question " + expr.column + " not found"));
    }
    answer = data[question._id] || {};
    cbt = _.extend({}, ((ref = answer.value) != null ? ref.cbt : void 0) || {}, {
      mpn: value
    });
    return callback(null, this.setValue(data, question, _.extend({}, answer.value || {}, {
      cbt: cbt
  ...
  ResponseDataExprValueUpdater.prototype.updateCBTConfidence = function(data, expr, value, callback) {
    var answer, cbt, question, ref;
    question = this.formItems[expr.column.match(/^data:([^:]+):value:cbt:confidence$/)[1]];
    if (!question) {
      return callback(new Error("Question " + expr.column + " not found"));
    }
    answer = data[question._id] || {};
  ...
  ResponseDataExprValueUpdater.prototype.updateCBTHealthRisk = function(data, expr, value, callback) {
    var answer, cbt, question, ref;
    question = this.formItems[expr.column.match(/^data:([^:]+):value:cbt:healthRisk$/)[1]];
    if (!question) {
      return callback(new Error("Question " + expr.column + " not found"));
    }
    answer = data[question._id] || {};
