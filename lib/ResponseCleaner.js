...
module.exports = ResponseCleaner = /*#__PURE__*/function () {
  function ResponseCleaner() {
    (0, _classCallCheck2["default"])(this, ResponseCleaner);
  ...
  (0, _createClass2["default"])(ResponseCleaner, [{
    key: "cleanData",
    value: function cleanData(design, visibilityCalculator, defaultValueApplier, randomAskedCalculator, data, responseRowFactory, oldVisibilityStructure, callback) {
      ...
      complete = false;
      newData = data;
      newVisibilityStructure = null; // This needs to be repeated until it stabilizes
      ...
      return async.whilst(function () {
        ...
        return visibilityCalculator.createVisibilityStructure(newData, responseRowFactory(newData), function (error, visibilityStructure) {
          ...
          newVisibilityStructure = visibilityStructure; // Clean data
          ...
          newData = _this.cleanDataCascadingLists(newData, newVisibilityStructure, design); // Default values
          ...
          nbIterations++; // If the visibilityStructure is still the same twice, the process is now stable.
    ...
    value: function cleanDataBasedOnVisibility(data, visibilityStructure) {
      ...
      for (key in visibilityStructure) {
        ...
        if (!visible) {
          values = key.split('.'); // If the key doesn't contain any '.', simply remove the data entry unless has randomAsked
          ...
          if (values.length === 1) {
            ...
            rosterGroupId = values[0]; // The index of the answer

            index = parseInt(values[1]); // The id of the answered question

            questionId = values[2]; // If a data entry exist for that roster and that answer index
    ...
    value: function cleanDataBasedOnChoiceConditions(data, visibilityStructure, design) {
      ...
      for (key in visibilityStructure) {
        ...
        if (visible) {
          values = key.split('.');
          selectedChoice = null; // FIRST: Setup what is needed for the cleaning the data (different for rosters)
          // If the key doesn't contain any '.', simply remove the data entry

          if (values.length === 1) {
            questionId = key;
            conditionData = newData;
            selectedChoice = (ref = newData[questionId]) != null ? ref.value : void 0; // A simple delete
            ...
            if (newData[rosterGroupId] != null && newData[rosterGroupId][index] != null) {
              ...
              deleteAnswer = function deleteAnswer() {
                var answerToClean; // Need to find what needs to be cleaned first (with roster data)
          ...
          if (selectedChoice != null) {
            // Get the question
            question = formUtils.findItem(design, questionId); // Of dropdown or radio type (types with conditional choices)
            ...
            if (question._type === 'DropdownQuestion' || question._type === 'RadioQuestion' || question._type === 'DropdownColumnQuestion') {
              ...
              for (i = 0, len = ref2.length; i < len; i++) {
                choice = ref2[i]; // If one of the choice is conditional
    ...
    key: "cleanDataCascadingLists",
    value: function cleanDataCascadingLists(data, visibilityStructure, design) {
      var answerValue, index, key, newData, question, questionId, ref, ref1, relevantData, rosterGroupId, row, rows, value, values, visible;
      newData = _.cloneDeep(data);
      ...
      for (key in visibilityStructure) {
        ...
        if (visible) {
          values = key.split('.');
          answerValue = null; // FIRST: Setup what is needed for the cleaning the data (different for rosters)
          // Simple case

          if (values.length === 1) {
            questionId = key;
            relevantData = newData;
            answerValue = (ref = newData[questionId]) != null ? ref.value : void 0; // A simple delete
            // Check if value is an array, which indicates roster
          } else if (_.isArray(newData[values[0]])) {
            // The id of the roster containing the data
            rosterGroupId = values[0]; // The index of the answer
            ...
            if (newData[rosterGroupId] != null && newData[rosterGroupId][index] != null) {
              ...
              answerValue = relevantData != null ? (ref1 = relevantData[questionId]) != null ? ref1.value : void 0 : void 0;
            }
          } // SECOND: look for conditional choices and delete their answer if the conditions are false
          ...
          if (answerValue != null) {
            // Get the question
            question = formUtils.findItem(design, questionId); // If cascading list

            if (question._type === 'CascadingListQuestion') {
              // If id, find row
              if (answerValue.id) {
                row = _.find(question.rows, {
                  id: answerValue.id
                });
                ...
                if (!row) {
                  ...
                  if (!!_.isEqual(answerValue, row)) {
                    relevantData[question._id].value = row; // Look up by column values as id is not present
                  }
                }
              } else {
