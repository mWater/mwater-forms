...
var PromiseExprEvaluator, VisibilityCalculator, _, async, conditionUtils, formUtils;
...
module.exports = VisibilityCalculator = /*#__PURE__*/function () {
  function VisibilityCalculator(formDesign, schema) {
    (0, _classCallCheck2["default"])(this, VisibilityCalculator);
  ...
  (0, _createClass2["default"])(VisibilityCalculator, [{
    ...
    value: function processGroup(item, forceToInvisible, data, responseRow, visibilityStructure, prefix, callback) {
      ...
      var applyResult, conditions, isVisible; // Once visibility is calculated, call this
    ...
    value: function processItem(item, forceToInvisible, data, responseRow, visibilityStructure, prefix, callback) {
      ...
      } else if ((ref = item._type) === 'TextColumn' || ref === 'Calculation') {
        return this.processQuestion(item, forceToInvisible, data, responseRow, visibilityStructure, prefix, callback);
      ...
      } else if ((ref1 = item._type) === 'Section' || ref1 === "Group" || ref1 === "Form") {
    ...
    value: function processQuestion(question, forceToInvisible, data, responseRow, visibilityStructure, prefix, callback) {
      ...
      var applyResult, conditions, isVisible, ref; // Once visibility is calculated, call this
        ...
        if (question._type === 'MatrixQuestion') {
          return async.each(question.items, function (item, cb) {
            return async.each(question.columns, function (column, cb2) {
              var newPrefix;
              newPrefix = "".concat(question._id, ".").concat(item.id, ".");
      ...
      if (question.conditionExpr) {
        return new PromiseExprEvaluator({
          schema: this.schema
        }).evaluate(question.conditionExpr, {
          row: responseRow
        }).then(function (value) {
          // Null or false is not visible
          if (!value) {
    ...
    value: function processRoster(rosterGroup, forceToInvisible, data, responseRow, visibilityStructure, prefix, callback) {
      ...
      applyResult = function applyResult(isVisible) {
        var dataId, subData;
        visibilityStructure[rosterGroup._id] = isVisible; // The data used (and passed down to sub items) is the one specified by rosterId if set
        ...
        if (subData != null) {
          // Get subrows
          return responseRow.followJoin("data:".concat(dataId)).then(function (rosterRows) {
            ...
            return async.forEachOf(subData, function (entry, index, cb) {
              return async.each(rosterGroup.contents, function (item, cb2) {
                var newPrefix;
                newPrefix = "".concat(dataId, ".").concat(index, "."); // Data is actually stored in .data subfield
      ...
      if (rosterGroup.conditionExpr) {
        return new PromiseExprEvaluator({
          schema: this.schema
        }).evaluate(rosterGroup.conditionExpr, {
          row: responseRow
        }).then(function (value) {
          // Null or false is not visible
          if (!value) {
