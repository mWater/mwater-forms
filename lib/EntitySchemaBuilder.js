...
exports.default = EntitySchemaBuilder = class EntitySchemaBuilder {
    // Pass in:
    ...
    addEntities(schema, entityTypes, propFilter, regionTypes) {
            ...
            traverseTree(entityType.properties, (prop) => {
                ...
                if (prop.type === "id") {
                    if (prop.idTable.match(/^entities\./)) {
                        ...
                        // Check that exists
                        if (!lodash_1.default.findWhere(entityTypes, { code: entityCode })) {
                            return;
                        }
                        reverseJoins.push({
                            table: prop.idTable,
        ...
        for (entityType of entityTypes) {
            ...
            contents = mapTree(entityType.properties || [], (prop) => {
                ...
                prop = lodash_1.default.pick(prop, "id", "name", "code", "desc", "type", "idTable", "enumValues", "deprecated", "expr");
                // Pad date fields
                if (prop.type === "date") {
                    // rpad(field ,10, '-01-01')
                    prop.jsonql = {
        ...
        const object = lodash_1.default.groupBy(reverseJoins, "table");
        for (let rjTable in object) {
            const rjs = object[rjTable];
            table = schema.getTable(rjTable);
            table.contents = table.contents.slice();
            let linksSection = lodash_1.default.findWhere(table.contents, { id: "!related_entities", type: "section" });
            if (!linksSection) {
                linksSection = {
                    id: "!related_entities",
                    type: "section",
            ...
            linksSection.contents = linksSection.contents.concat(lodash_1.default.pluck(rjs, "column"));
            schema = schema.addTable(table);
        }
        return schema;
    }
...
function mapTree(tree, func) {
    ...
    return lodash_1.default.compact(lodash_1.default.flatten(lodash_1.default.map(tree, function (item) {
        let newItem = func(item);
        if (newItem && item.contents) {
            newItem = lodash_1.default.extend({}, newItem, { contents: mapTree(item.contents, func) });
        }
        return newItem;
    })));
}
...
function pluralize(lstr) {
    ...
    return lodash_1.default.extend({}, lstr, { en: pstr });
}
