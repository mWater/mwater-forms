    ...
    value: function addEntities(schema, entityTypes, propFilter) {
      ...
      for (j = 0, len1 = entityTypes.length; j < len1; j++) {
        ...
        contents = _mapTree(entityType.properties || [], function (prop) {
          ...
          prop = _.pick(prop, "id", "name", "code", "desc", "type", "idTable", "enumValues", "deprecated"); // Don't include roles

          delete prop.roles; // Convert id to join
        ...
        contents.push({
          ...
          desc: {
            en: "True if there are verifications pending for this site that have not been verified"
          },
          type: 'boolean'
        }); // Add datasets
