...
module.exports = EntitySchemaBuilder = (function() {
  ...
  EntitySchemaBuilder.prototype.addEntities = function(schema, entityTypes, properties) {
    ...
    for (j = 0, len1 = entityTypes.length; j < len1; j++) {
      ...
      contents = [];
      labelColumn = null;
      entityProps = _.cloneDeep(_.filter(properties, function(prop) {
      ...
      for (k = 0, len2 = entityProps.length; k < len2; k++) {
        ...
        if (prop.code === "location") {
          ...
          prop.desc = {
            _base: "en",
            en: "Latitude/longitude of the site. Use 'Location' instead for filtering by country, state, etc."
          };
        }
        if (prop.code === "admin_region") {
          ...
          prop.desc = {
            _base: "en",
            en: "Administrative region (country, state/province, district, village etc.)"
          };
        }
        if (prop.unique_code) {
