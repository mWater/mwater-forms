...
class RosterGroupComponent extends react_1.default.Component {
    ...
    renderEntry(entry, index) {
        ...
        return R("div", { key: index, className: "panel panel-default" }, R("div", { key: "header", className: "panel-heading", style: { fontWeight: "bold", position: "relative" } }, `${index + 1}. `, this.renderEntryTitle(entry, index), R("button", {
            className: "btn btn-link",
            style: { position: 'absolute', right: 0, top: 5 },
            onClick: this.handleToggle.bind(null, index)
        }, R("span", { className: classnames_1.default('glyphicon', { 'glyphicon-chevron-up': !isCollapsed, 'glyphicon-chevron-down': isCollapsed }) }))), R("div", { key: "body", className: "panel-body", style: bodyStyle }, this.props.rosterGroup.allowRemove
            ? R("button", {
                type: "button",
                style: { float: "right" },
    ...
    renderAdd() {
        if (this.props.rosterGroup.allowAdd) {
            return R("div", { key: "add" }, R("button", { type: "button", className: "btn btn-default btn-sm", onClick: this.handleAdd }, R("span", { className: "glyphicon glyphicon-plus" }), " " + this.context.T("Add")));
        }
    }
    renderEmptyPrompt() {
