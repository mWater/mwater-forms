...
describe("RosterMatrixComponent", function() {
  ...
  it("removes entry when remove is clicked", function(done) {
    ...
    const comp = this.render({rosterMatrix: this.rosterMatrix, data: { a: [{ _id: "1", data: { x: { value: 1 } } }, { _id: "2", data: { x: { value: 2 } } }] }, onDataChange});
  ...
  it("puts answers from column components in correct position in array", function(done) {
    ...
    const comp = this.render({rosterMatrix: this.rosterMatrix, data: { a: [{ _id: "1", data: {}}, { _id: "2", data: {}}] }, onDataChange});
  ...
  it("requires required columns fail", function() {
    ...
    return assert.isTrue(ReactTestUtils.findRenderedComponentWithType(comp.getComponent(), RosterMatrixComponent).validate(false), "Should fail validation");
  });

  it("requires required columns pass", function() {
    this.rosterMatrix.contents[0].required = true;

    const comp = this.render({rosterMatrix: this.rosterMatrix, data: { a: [{ _id: "1", data: { text: { value: "x" }}}] }});
    return assert.isFalse(ReactTestUtils.findRenderedComponentWithType(comp.getComponent(), RosterMatrixComponent).validate(false), "Should pass validation");
  });
  ...
  return it("validates columns pass", function() {
    ...
    const comp = this.render({rosterMatrix: this.rosterMatrix, data: { a: [{ _id: "1", data: { text: { value: "12345" }}}] }});
    return assert.isFalse(ReactTestUtils.findRenderedComponentWithType(comp.getComponent(), RosterMatrixComponent).validate(false), "Should pass validation");
  });
});
