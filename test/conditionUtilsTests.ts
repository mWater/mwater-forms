...
describe("conditionUtils", function () {
  ...
  describe("compileConditions", () =>
    it("combines conditions", function () {
      const data = {
        lhsid1: { value: 1 },
        lhsid2: { value: 2 }
      }
  ...
  describe("applicableOps", function () {
    ...
    it("is correct for Question with N/A alternate", function () {
      ...
      return assert.deepEqual(_.pluck(ops, "id"), [
        "present",
        "!present",
        "contains",
        "!contains",
    ...
    it("is correct for MulticheckQuestion with N/A alternate", function () {
      ...
      return assert.deepEqual(
        _.pluck(ops, "id"),
        ["present", "!present", "includes", "!includes", "isoneof", "isntoneof"],
        "Same as does not use is/isn't"
      )
    })
  ...
  describe("rhsChoices", function () {
    it("is correct for DropdownQuestion", function () {
      ...
      return assert.deepEqual(choices, [
        { id: "id1", text: "First" },
        { id: "id2", text: "Segundo" }
      ])
    })
  ...
  describe("validateCondition", function () {
    ...
    it("fails for no lhs", function () {
      return assert.isFalse(conditionUtils.validateCondition({}, this.form))
    })

    it("fails for non-existant question", function () {
      return assert.isFalse(
        conditionUtils.validateCondition(
          {
            lhs: { question: "xxx" }
          },
          this.form
  ...
  return describe("summarizeConditions", function () {
    ...
    it("unary condition", function () {
      return assert.equal(
        conditionUtils.summarizeConditions(
          [
            {
              lhs: { question: "002" },
              op: "present"
