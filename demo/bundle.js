...
LocationFinder = (function() {
  var cacheLocation, getCachedLocation;

  function LocationFinder() {
    _.extend(this, Backbone.Events);
  }

  cacheLocation = function(pos) {
    return window.localStorage['LocationFinder.lastPosition'] = JSON.stringify(pos);
  };
...
LocationView = (function(_super) {
  ...
  function LocationView(options) {
    this.getAccuracyAttributes = __bind(this.getAccuracyAttributes, this);
  ...
  LocationView.prototype.render = function() {
    ...
    accuracy = this.getAccuracyAttributes(this.currentLoc);
    this.$("#location_set").attr("disabled", this.settingLocation || this.readonly).removeClass("disabled btn-danger btn-warning btn-success").addClass(accuracy["class"]);
    console.log(accuracy.strength);
    if (accuracy.strength === "weak") {
      return this.$("#location_set").text("Waiting for GPS...");
    }
  };
  ...
  LocationView.prototype.saveEditLocation = function() {
    if (isNaN(parseFloat(this.$("#latitude").val()))) {
      alert("Invalid latitude");
      return;
    }
    if (isNaN(parseFloat(this.$("#longitude").val()))) {
  ...
  LocationView.prototype.getAccuracyAttributes = function(pos) {
    if (!(pos && pos.accuracy)) {
      return {
        color: "red",
