...
export default class LocationFinder {
  ...
  cacheLocation(pos: any) {
    if (this.storage != null) {
      // Store last position. GeolocationPosition is not JSON serializable, 
      // so we need to extract the data we need
      this.storage.set("LocationFinder.lastPosition", JSON.stringify({
        coords: {
  ...
  getCachedLocation() {
    if (this.storage != null && this.storage.get("LocationFinder.lastPosition")) {
      ...
      // Check that valid position (https://stackoverflow.com/questions/11042212/ff-13-ie-9-json-stringify-geolocation-object)
      if (!pos.coords) {
        return
      }
