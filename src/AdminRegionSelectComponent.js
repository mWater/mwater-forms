...
export default AdminRegionSelectComponent = (function() {
  AdminRegionSelectComponent = class AdminRegionSelectComponent extends AsyncLoadComponent {
    ...
    load(props, prevProps, callback) {
      ...
      if (props.value) {
        return props.getAdminRegionPath(props.value, (error, path) => {
          ...
          callback({error: null, path, busy: false, level1s: null, level2s: null, level3s: null, level4s: null, level5s: null});
    ...
    renderLevel(level) {
      ...
      return R('tr', {key: level},
        R('td', {style: { paddingLeft: 10, paddingRight: 10 }, className: "text-muted"},
          this.state.path[level] ?
            this.state.path[level].type : undefined
        ),
        R('td', null,
          R('select', {key: `level${level}`, className: "form-control", value: (this.state.path[level] ? this.state.path[level].id : ""), onChange: this.handleChange.bind(null, level)},
            R('option', {key: "none", value: ""}, 
              this.state.path[level] ? this.props.T("None") : this.props.T("Select...")),

            (() => {
            if (this.state[`level${level}s`]) {
              return _.map(this.state[`level${level}s`], subRegion => R('option', {key: subRegion.id, value: subRegion.id}, subRegion.name));
            } else if (this.state.path[level]) { // No options yet, just use value
    ...
    render() {
      ...
      return R('table', {style: { opacity: (this.state.busy ? 0.5 : undefined) }},
        R('tbody', null, 
