...
export default AquagenxCBTPopupComponent = (function() {
  AquagenxCBTPopupComponent = class AquagenxCBTPopupComponent extends React.Component {
    ...
    componentDidMount() {
      ...
      return $( main ).find( "#compartment5" ).click(ev => {
    ...
    renderStyle() {
      ...
      const compartmentValues = [cbtValues.c1, cbtValues.c2, cbtValues.c3, cbtValues.c4, cbtValues.c5];
      const compartmentColors = _.map(compartmentValues, function(c) { if (c) { return '#32a89b'; } else { return '#ebe7c2'; } });
      const hoverColors = _.map(compartmentValues, function(c) { if (c) { return '#62c5bb'; } else { return '#fcf8d6'; } });
      return R('style', null,
        `\
    ...
    render() {
      return React.createElement(ModalPopupComponent, {
        footer: R('div', {id: 'footer'},
          R('button', {className: 'btn btn-primary', id: 'save', onClick: this.handleSaveClick}, this.context.T('Save')),
          R('button', {type: "button", className: "btn btn-default", id: 'close', onClick: this.props.onClose}, this.context.T('Cancel'))),
        header: this.context.T('Click on the compartments to change color')
      },
        R('div', {ref: (c => { return this.main = c; }), id: `cbtPopup${this.props.questionId}`},
          this.renderStyle(),
