...
import ResponseViewEditComponent from './ResponseViewEditComponent';
import { Schema } from 'mwater-expressions';
import FormSchemaBuilder from './FormSchemaBuilder';
import ImageUploaderModalComponent from './ImageUploaderModalComponent';
...
const formCtx = {
  ...
  imageManager: {
    ...
    getThumbnailImageUrl(id, success, error) { return error("Not implemented"); }
  },
  ...
  getCustomTableRow: (tableId, rowId) => { if (tableId === "custom.ts.cities") { return Promise.resolve(_.findWhere(cascadingRefRows, {_id: rowId})); } },
...
class DemoLocationEditorComponent extends React.Component {
  ...
  render() {
    return R(LocationEditorComponent, { 
      ...
      onUseMap: () => this.setState({location: { latitude: 46, longitude: -73, altitude: 240, accuracy: 12, altitudeAccuracy: 30, depth: 3 }}),
      T: global.T
    }
    );
  }
