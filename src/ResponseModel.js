...
export default ResponseModel = class ResponseModel {
  ...
  draft(deploymentId) {
    ...
    } else if (!this.response.deployment) { // Select first deployment if not specified and not already present
  ...
  redraft() {
    ...
    // Unfinalize if final
    if (this.response.status === "final") { this._unfinalize(); }
  ...
  fixRoles() {
    ...
    return this.response.roles = this.response.roles.concat(_.map(viewers, s => ({
      id: s,
      role: "view"
    })));
  }
  ...
  canApprove() {
    ...
    const admins = _.union(_.pluck(_.where(this.form.roles, { role: "admin"}), "id"), deployment.admins, deployment.approvalStages[this.response.approvals.length]?.approvers || []);
    let subjects = ["user:" + this.user, "all"];
    subjects = subjects.concat(_.map(this.groups, g => "group:" + g));
  ...
  canReject() {
    ...
    if (this.response.status === "pending") {
      // Get list of admins at both deployment and form level and add approvers
      admins = _.union(_.pluck(_.where(this.form.roles, { role: "admin"}), "id"), deployment.admins, deployment.approvalStages[this.response.approvals.length]?.approvers || []);
