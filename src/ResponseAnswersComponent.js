...
export default ResponseAnswersComponent = (function() {
  ResponseAnswersComponent = class ResponseAnswersComponent extends AsyncLoadComponent {
    static initClass() {
      this.propTypes = {
        ...
        hideCalculations: PropTypes.bool
      };
       // allows to hide the calculation section, used in assignments
    }
    ...
    load(props, prevProps, callback) {
      ...
      return new VisibilityCalculator(props.formDesign, props.schema).createVisibilityStructure(props.data, responseRow, (error, visibilityStructure) => {
    ...
    renderLocation(location) {
      if (location) {
        return R('div', null,
          R('a', {onClick: this.handleLocationClick.bind(this, location), style: { cursor: "pointer" }},
      ...
      switch (formUtils.getAnswerType(q)) {
        case "text":
          ...
          if (answer.value && answer.value.match(/^((([A-Za-z]{3,9}:(?:\/\/)?)(?:[\-;:,&=\+\$,\w]+@)?[A-Za-z0-9\.\-]+|(?:www\.|[\-;:,&=\+\$,\w]+@)[A-Za-z0-9\.\-]+)((?:\/[\+~%\/\.\w\-_]*)?\??(?:[\-\+=&,;%@\.\w_]*)#?(?:[\.\!\/\\\w]*))?)$/)) {
            ...
            return R('a', {href: answer.value, target},
              answer.value);
          }
        ...
        case "boolean":
          if (answer.value) { return this.props.T("True"); } else { return this.props.T("False"); }
        ...
        case "image":
          if (answer.value) {
            return R(ImageDisplayComponent, {image: answer.value, imageManager: this.props.formCtx.imageManager, T: this.props.T});
          }
          break;

        case "images":
          return _.map(answer.value, img => {
            return R(ImageDisplayComponent, {image: img, imageManager: this.props.formCtx.imageManager, T: this.props.T});
          });
    ...
    renderLikertAnswer(q, answer, prevAnswer) {
      ...
      if (formUtils.getAnswerType(q) === "items_choices") {
        const contents = [];
        for (let item of q.items) {
          const itemTd = R('td', {style: {textAlign: "center"}},
      ...
      return [
        R('tr', trProps,
          R('td', {key: "name", style: { width: "50%" }},
            formUtils.localizeString(q.text, this.props.locale)),
          R('td', {key: "value"},
            R('div', null,
              (likertAnswer == null) ?
                this.renderAnswer(q, answer, dataId) : undefined,
              (() => {
              ...
              answer && answer.location ?
                this.renderLocation(answer.location) : undefined,

              answer && answer.comments ?
                R('div', {className: "text-muted"},
                  answer.comments) : undefined,

              (prevAnswer != null) && !_.isEqual(prevAnswer.value, answer?.value) && this.props.showChangedLink ?
                R('a', {style: { float: 'right', display: 'inline-block', cursor: 'pointer', fontSize: 9 }, onClick: this.props.onChangedLinkClick, key: 'view_change'},
          ...
          this.props.showPrevAnswers && this.props.prevData ?
            R('td', {key: "prevValue"},
              (prevAnswer != null) && !_.isEqual(prevAnswer.value, answer?.value) && this.props.onCompleteHistoryLinkClick ?
                R('a', {style: { float: 'right', display: 'inline-block', cursor: 'pointer', fontSize: 9 }, onClick: this.props.onCompleteHistoryLinkClick, key: 'view_history'},
                  T("Show Changes")) : undefined,

              (typeof prevMatrixAnswer === 'undefined' || prevMatrixAnswer === null) ?
                this.renderAnswer(q, prevAnswer) : undefined,
              prevAnswer && prevAnswer.timestamp ?
                R('div', null,
                  this.props.T('Answered'),
                  ": ",
                  moment(prevAnswer.timestamp).format('llll')) : undefined,
              prevAnswer && prevAnswer.location ?
                this.renderLocation(prevAnswer.location) : undefined
            ) : undefined
        ),
        likertAnswer
      ];
    ...
    renderExpression(q, dataId) {
      return [
        R('tr', {key: dataId},
          ...
          R('td', {key: "value"},
            R('div', null,
              this.renderExpressionAnswer(q, dataId))
          ),

          this.props.showPrevAnswers && this.props.prevData ?
            R('td', {key: "prevValue"}, null) : undefined
        )
      ];
    }
    ...
    render() {
      ...
      return R('div', null,
        R('table', {className: "table table-bordered table-condensed", style: { marginBottom: 0 }},
          R('thead', null,
            R('tr', null,
              R('th', null, "Question"),
              R('th', null, "Answer"),
              this.props.showPrevAnswers ?
                R('th', null, "Original Answer") : undefined
            )
          ),
          R('tbody', null,
        ...
        this.props.formDesign.calculations && (this.props.formDesign.calculations.length > 0) && this.state.responseRow && !this.props.hideCalculations ?
  ...
  ResponseAnswersComponent.initClass();
